...
the nearest object to the vehicle in real time and control the chassis' rotation to track it.

Furthermore, if the object's distance to the vehicle falls below an alarm threshold, a buzzer on the

vehicle sounds an alarm.
...
start the radar tracking gameplay. If you do not start the handle control, it will not affect the

operation of this program. Enter the following command in the terminal to start the radar

tracking program,


After the program starts, the radar will scan for the nearest object within the detection range.

Slowly moving the object left or right will cause the chassis to rotate, tracking the object and

aligning the front of the vehicle with it. If the distance between the vehicle and the object is less

than 0.55 meters, the onboard buzzer will sound.
