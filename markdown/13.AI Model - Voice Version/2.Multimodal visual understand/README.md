...
## 1. Course Content
Basic: Run the example program, allowing the robot to observe the environment and execute
...
need to open a terminal on the host machine and then enter the command to access the Docker

container. After entering the Docker container, enter the commands mentioned in this section in

the terminal. For instructions on accessing the Docker container from the host machine, please

refer to the "Accessing the Robot's Docker (For Jetson Nano and Raspberry Pi 5 users)" section in

the product tutorial [0. Instructions and Installation Steps]. For Orin and NX boards, simply open a

terminal and enter the commands mentioned in this section.
...
![](Multimodal-visual-understand.pdf-1-0.jpeg)
## 3. Running Examples
### 3.1 Starting the Program
...
![](Multimodal-visual-understand.pdf-1-3.jpeg)
### 3.2 Test Cases
These test cases are for demonstration purposes only; users can create their own dialogue
...
Please check if there is a blue cube and a pack of tissues in front of you. If there is, nod your

head; if not, shake your head.

#### 3.2.1 Case 1
First, use "Hello yahboom" to wake up the robot. The robot will respond. After the recording
...
speaking, it will perform end-of-speech detection. If there is silence for more than 1.5

seconds, the recording will stop. - The robot will first respond to the user, then perform the

actions according to the instructions, while simultaneously printing the following information
...
![](Multimodal-visual-understand.pdf-2-0.jpeg)
![](Multimodal-visual-understand.pdf-3-0.jpeg)
...
The robot has short-term memory. After being activated, all interactions will be stored

in the AI large language model's historical context memory. The robot will only clear its

previous memory when the user explicitly requests to end the current task or gives
...
![](Multimodal-visual-understand.pdf-3-1.jpeg)
...
![](Multimodal-visual-understand.pdf-4-0.jpeg)


Model service source code:


The main program that implements the robot's visual observation function is the `seewhat`


The function saves and displays an image from the latest perspective.


feedback to the multi_brains agent in Dify.
...
```
  ...
  def __display_saved_image(self):
     ...
     """
     显示已保存的图片 4 秒后关闭窗口 / Display the saved image for 4 seconds before
...
"""
     ...
     try:
       ...
       if img is not None:
          ...
          cv2.waitKey(4000) # 等待 4 秒 / Wait for 4 seconds
...
requests to access the `multi_brains` agent.

If the `llm_request` field in the request message indicates an image request, a list

`[msg.llm_request, 'image_request', True]` is constructed and added to the model
