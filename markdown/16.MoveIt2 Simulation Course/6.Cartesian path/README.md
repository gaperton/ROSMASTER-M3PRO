# Cartesian path
Preface: ROS on Raspberry Pi 5 and Jetson-nano runs in Docker, so the performance of running

MoveIt2 is average. It is recommended that users of Raspberry Pi 5 and Jetson-nano

motherboards run MoveIt2 related cases in a virtual machine. ROS on Orin motherboard runs

directly on the motherboard, so users of Orin motherboard can run MoveIt2 related cases directly

on the motherboard. The instructions are the same as running in a virtual machine.


The following content uses running on a virtual machine as an example.

## 1. Content Description
This section explains how to use functions in MoveIt2 to implement Cartesian paths. Cartesian

path planning is an important feature in robotic arm control. It allows users to specify the

trajectory of the end effector in Cartesian space (i.e., three-dimensional space) rather than directly

specifying the motion in joint space. MoveIt provides powerful Cartesian path planning
...
Then, we need to add a plug-in to display the planned trajectory, and set it up as shown in the

figure below.


![](6.Cartesian-path.pdf-0-0.jpeg)
![](6.Cartesian-path.pdf-1-0.jpeg)

Next, we modify the topics that need to be displayed, as shown below.


![](6.Cartesian-path.pdf-2-0.jpeg)

Then, enter the following command in the virtual machine terminal to start the Cartesian path

program,


After running, the robot arm will move to the set posture and a green Cartesian path point will be

displayed, as shown in the figure below.


![](6.Cartesian-path.pdf-3-0.jpeg)
