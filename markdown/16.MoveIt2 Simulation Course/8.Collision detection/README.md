...
This lesson explains how to use the MoveIt library to add obstacles to rviz and implement

obstacle avoidance when MoveIt plans motion.
...
Then, we enter the following command in the virtual machine terminal to start the collision

detection program,


![](8.Collision-detection.pdf-0-1.jpeg)


After the program is run, a green block will be added to rviz. The robot arm will first move to the

up position, then plan a movement to the down position, avoiding obstacles during the process;

finally, it will plan a movement from the down position to the up position, also avoiding obstacles

during the process. As shown in the figure below,
...
![](8.Collision-detection.pdf-1-0.jpeg)
```
