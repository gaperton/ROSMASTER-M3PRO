...
## 1. Course Content
Run the example program, allowing the robot to observe the environment through text

interaction in the terminal and perform tasks based on instructions.


[!NOTE]


The only difference between the text version and the voice version is the method of

instruction input; the text version does not include speech recognition and speech
...
![](Multimodal-visual-understand.pdf-0-1.jpeg)
## 3. Running the Examples
### 3.1 Starting the Program
...
![](Multimodal-visual-understand.pdf-1-2.jpeg)
### 3.2 Test Cases
...
Please look to see if there is a blue block and a pack of tissues in front of you. If there is, nod

your head; if not, shake your head.

#### 3.2.1 Case 1
Enter the test case in the text interaction terminal. After the model thinks, the model will reply to

the user and perform the actions according to the user's instructions.


![](Multimodal-visual-understand.pdf-2-0.jpeg)

After the robot completes a task, it enters a waiting state. At this point, you can continue the

conversation. The instructions will be directly passed to the execution layer's large language

model, and all conversation history will be retained. You can enter the "end current task"

command again to have the robot end the current task cycle and start a new one.

#### 3.2.2 Case 2
Similar to Case 1, entering Case 2 in the terminal will cause the model to respond and perform

actions according to the instructions.


![](Multimodal-visual-understand.pdf-2-1.jpeg)
## 4. Source Code Analysis
...
to the multi_brains agent in Dify.


![](Multimodal-visual-understand.pdf-3-4.jpeg)
...
![](Multimodal-visual-understand.pdf-4-1.jpeg)


only text responses will be provided.
...
When initializing the model engine, the speech recognition and speech synthesis models are

loaded only in speech interaction mode.
