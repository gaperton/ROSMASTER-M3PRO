...
The above completes the installation of VMWare. You can open VMWare on the desktop, as shown below.
...
After installing the VMware Workstation virtual machine, double-click to open it and enter the interface as shown in the figure. Click [Open Virtual Machine]
...
If the prompt shown in the figure below appears, directly select [I have copied this virtual machine (p)].
...
Processor: The higher the number of cores (c) per processor, the smoother the virtual machine will run, but it must not exceed the total number.
