# Compile the micro-ROS Driver Library

## 1. Install the Cross-Compiler

Open the download page and download the Linux version of the Arm GNU toolchain:

https://developer.arm.com/downloads/-/gnu-rm

After extracting the archive, place the toolchain files in:

/opt/gcc-arm-none-eabi

Add the executable path to `.bashrc` in the user directory:

export PATH=\$PATH:/opt/gcc-arm-none-eabi/bin

## 2. Get the STM32 micro-ROS Library File

Download and build the micro-ROS setup tool:
...
Use the micro-ROS setup tool to generate the STM32 micro-ROS library file:

```bash
ros2 run micro_ros_setup create_firmware_ws.sh generate_lib
```

Check the workspace. It should now contain five folders: `build`, `firmware`, `install`, `log`, and `src`. The `firmware` folder contains the STM32 micro-ROS library files.

## 3. Modify Parameters

Open `toolchain.cmake`:
...
Create a new `microros` folder in the project, then copy the `include` and `libmicroros` folders generated by the previous step into it.
