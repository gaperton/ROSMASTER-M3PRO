...
- 2. Next, create an action folder under the pkg_interfaces function package and create a new file called [Progress.action] within the action folder. The file contents are as follows:
...
After executing the above command, the pkg_action function package will be created, along with an action_server_demo node and the relevant configuration files.

## 4. Server Implementation

### 4.1 Creating the Server

Next, edit [action_server_demo.py] to implement the server functionality and add the following code:
...
feedback_msg.progress = i / goal_handle.request.num
            self.get_logger().info(': %.2f' % feedback_msg.progress)
...
Next, edit action_client_demo.py to implement the server functionality and add the following code:
...
self.get_logger().info(':sum = %d' % result.sum)
    ...
    def feedback_callback(self, feedback_msg):
        ...
        self.get_logger().info(': %d%%' % feedback)
def main(args=None):
    rclpy.init(args=args)
