...
- **Surfacing `heading_path` to first-stage retrieval is the single biggest lift the harness has measured.** Prepending the heading path to chunk text before embedding (so the dense vector carries section context) and adding `heading_path` as a weighted 2× FTS5 column (so BM25 also sees it) moved file MRR to ceiling (0.950 → 1.000), chunk MRR up to 0.600/0.562 (from 0.531/0.529 baseline; better than bge-large's 0.483/0.483), and gave +10 clear / +2 slight noob wins against 2 clear / 4 slight regressions. Sqlite latency dropped to ~38 ms; lance to ~105 ms. Same bge-small weights, no new dependency. Confirms Q6: bge-large in NX2 was reinventing the heading signal that the schema was silently withholding. *(Experiment 10.)*
...
- **Promote NX8 (heading_path in both retrieval channels) to the default index format.**  
  *Why:* Experiment 10 produced the best labeled and subjective numbers the harness has measured (file MRR 1.000, chunk MRR 0.600/0.562, +10/+2 vs. −2/−4 on noob) at no additional dependency or model cost. The variant lives at `*.h.*` paths; promoting means making it the path the default `small` variant points at. `rag_query.py` already accepts both schemas via the FTS column-count probe in `_fts_bm25_expr`.  
  *Revisit if:* the heading-weight tuning follow-up (Q7) finds a weight that fixes the NQ4 / NQ17 regressions, in which case the schema stays and only the bm25 weight changes.
...
- **Q7. What heading BM25 weight minimises over-anchoring without giving up NX8's wins?**  
  NX8 used `bm25(fts_chunks, 2.0, 1.0)`. Two regressions trace to this weight being too strong: NQ4 ("map a room") promoted *Multimodal visual + SLAM navigation* over the actual SLAM mapping tutorials because "SLAM" appears in the heading; NQ17 ("drive straight for 1 m") promoted "Robotic Arm Calibration — Calibrate the median value" because "Calibrate" matched semantically. Lower weights (1.5? 1.25?) may keep the +10 wins while recovering the −2.  
  *Test:* NX10 — sweep weights {1.0, 1.25, 1.5, 1.75, 2.0} on the small_h index, rerun labeled + noob, pick the Pareto frontier.
...
2. **NX10 — Sweep heading BM25 weight on `small_h`.**  
   Parameterise `_fts_bm25_expr` in [sqlite-rag/rag_query.py](sqlite-rag/rag_query.py) to take a configurable heading weight, run `evaluate.py --mode hybrid` and `ask.py --file noob_questions.txt` at {1.0, 1.25, 1.5, 1.75, 2.0}, record the Pareto frontier of labeled chunk MRR vs. noob NQ4/NQ17 recovery. Pick the weight that keeps NX8's +10 noob wins while recovering NQ4 (map-a-room) and NQ17 (drive-straight).  
   *Resolves:* Q7. *Fast (no re-embed), targets the only documented downsides of NX8.*

3. **NX9 — Boundary-aware chunking.**  
...
4. **NX3 — Add multi-query expansion.**  
