...
- **Boundary-aware chunking is cleaner but not a retrieval-quality win yet.** NX9 removed page-span/link litter and stopped splitting inside fenced code blocks and Markdown tables, but the rebuilt `small_h` indexes dropped labeled chunk MRR from 0.613/0.562 to 0.548/0.546 and reintroduced important noob regressions: NQ4 map-room lost the Cartographer top rank again, and NQ20 velocity-command moved behind angular-velocity calibration. File MRR stayed at 1.000/1.000, so the damage is intra-file ranking rather than file recall. *(Experiment 12.)*

- **Narrow local routing fixes predictable beginner-query misses without hurting the labeled benchmark.** NX13 adds offline keyword expansion plus small post-RRF route boosts for mapping, chassis velocity, and selected ROS2 workflow intents. After tightening exact mapper handling and avoiding broad `publisher` / `topic` expansion, labeled metrics returned exactly to the NX10 baseline (file MRR 1.000/1.000, chunk MRR 0.613/0.562), while NQ4 map-room now ranks `Cartographer-SLAM mapping` first on both engines and NQ17 drive-straight now ranks `ROS control > Controlling the Car's Speed` first on both engines. *(Experiment 13.)*
...
- **Promote the NX8 heading-path schema, but not NX9's boundary-aware chunking result, as the quality baseline.**  
...
| 12 | Does boundary-aware chunking improve retrieval specificity? | Rewrote both indexers to clean PDF page spans / page-anchor links, split by paragraph then sentence then character fallback, and keep fenced code blocks / Markdown tables atomic. Rebuilt both `small_h` indexes: 247 markdown files, 246 indexed files, **2,185 chunks**. Reran `tests/evaluate.py --variant small_h --mode hybrid` and `tests/ask.py --variant small_h --mode hybrid --file tests/noob_questions.txt`. | No. File MRR stayed **1.000 / 1.000**, but chunk MRR fell **0.613/0.562 → 0.548/0.546**. The cleaned chunks are nicer to read and no longer contain `<span id="page-…"></span>` / page-anchor wrappers, but ranking got worse: `angular-velocity-cal` moved from chunk rank 1 to 3, sqlite NQ4 regressed from Cartographer rank 1 back to multimodal SLAM, and NQ20 regressed from `ROS control > Controlling the Car's Speed` to angular velocity calibration. Top-1 changed on 15/30 noob questions versus the old `small` baseline for both engines, with a mix of clear wins (NQ8, NQ13, NQ16, NQ19, NQ22) and clear losses (NQ4, NQ17, NQ20). Resolves Q4: boundary-aware chunking as implemented is hygiene-positive but quality-negative. |
| 13 | Can deterministic local routing fix the remaining predictable noob misses without giving up NX10's benchmark gains? | Restored both `small_h` indexes to legacy NX8/NX10 chunking, made chunking selectable (`--chunking legacy|boundary`) so NX9 remains reproducible, then added a shared query-profile pattern to both query engines: original query for vector search, expanded query for keyword search, and small post-RRF route boosts for mapping, chassis velocity, and narrow ROS2 workflow intents. Tuned after an initial over-broad ROS2 pass regressed `ros2-publisher`. Reran `tests/evaluate.py --variant small_h --mode hybrid`, targeted asks for NQ4/NQ17/NQ20, and full `tests/ask.py --variant small_h --mode hybrid --file tests/noob_questions.txt`. | Yes, with narrow rules. Labeled metrics are unchanged from NX10: file MRR **1.000 / 1.000**, chunk MRR **0.613 / 0.562**, chunk-match@1 **5/10 / 4/10**. NQ4 now ranks `6.Lidar Course/7.Cartographer-SLAM mapping` first on both engines; NQ17 now ranks `5.Chassis Control Course/1.ROS control > Controlling the Car's Speed` first on both engines; NQ20 stays on `ROS control`. Only six noob questions trigger the rule layer (NQ4, NQ14, NQ16, NQ17, NQ19, NQ20), which limits blast radius. New rule: keep routing narrow, exact when possible, and post-fusion. |
