...
This section is exclusive to the Orin motherboard and primarily introduces optimizing the Yolov8 object detection framework using TensorRT. Users control the robot to track a target by selecting a track\_id.
...
Then, based on the ID in the image, enter the following command to publish to the target topic /track\_id. For example, if we want to track an object with ID 4, enter the following command and press Enter to publish the message.
